...
if(isset($_POST['submit']))
{
		...
		$_SESSION['log']=3;
		$sql="select * from ACN_Users where strUserID = '".$userID."' and strWebpw = '".$pincode."'";
 ...
 //GET Name instead user.
   $sql2= "select * from ACN_Users where strUserID = '".$userID."'";
   if(($result=odbc_exec($con,$sql2))=== false )		//Run query and validate.
     die("Query error." .odbc_errormsg($sql2));		//Run query and validate.
   {
		 $row = odbc_fetch_array($result);
		 $analystName = $row['strUsername']; // Get Analyst Name from database
     $userID = $row['strEmailname']; // Get Analyst ID from database
		 $analystTeam = $row['strUserTeam']; // Get Analyst Team from database
